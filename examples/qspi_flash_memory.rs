...
use stm32h7xx_hal::{
    pac, prelude::*, xspi::Config, xspi::Qspi, xspi::QspiMode, xspi::QspiWord,
};
...
fn main() -> ! {
    ...
    // Enable quad mode to fully utilize QSPI
    enable_qpi_mode(&mut qspi);
...
fn enable_qpi_mode(qspi: &mut Qspi<pac::QUADSPI>) {
    enable_write_operation(qspi);
